...
asset_of_interest = 'LED-driver'


######################################################################
### Print meta_info as the Asset
######################################################################
type_uri = 'https://wegenenverkeer.data.vlaanderen.be/ns/onderdeel#LEDDriver'
...
df_input_asset_of_interest = df_input_asset_of_interest.iloc[:,:]
...
df_relations_HoortBij = df_relations[
    (df_relations['AIMDBStatus.isActief'] == True)  # actieve relaties
    &
    (
        (df_relations['RelatieObject.typeURI'].str.endswith('Sturing'))
        |
        (df_relations['RelatieObject.typeURI'].str.endswith('Bevestiging'))
    )
    &
    df_relations['RelatieObject.doel'].apply(lambda x: x['@type'].endswith('#VerlichtingstoestelLED'))  # doel-asset
]


# Normalize de json-kolom van het dataframe.
df_relaties_bron_normalized = pd.json_normalize(df_relations_HoortBij['RelatieObject.bron']).set_index(df_relations_HoortBij.index)
df_relaties_doel_normalized = pd.json_normalize(df_relations_HoortBij['RelatieObject.doel']).set_index(df_relations_HoortBij.index)
...
df_relations_HoortBij = pd.concat([df_relations_HoortBij, df_relaties_bron_normalized], axis=1)
df_relations_HoortBij = pd.concat([df_relations_HoortBij, df_relaties_doel_normalized], axis=1)


# Lanceer de API-request om de geometrie op te halen van bron-assets en doel-assets van de relaties
# Bron-asset
bron_asset_uuids = df_relations_HoortBij['RelatieObject.bron.uuid']
...
# Doel-asset
doel_asset_uuids = df_relations_HoortBij['RelatieObject.doel.uuid']
...
# Merge de dataframes en link zo de wkt-geometrie aan de originele asset: df_bron_asset; df_doel_asset; df_relations_HoortBij
df_output = pd.merge(left=df_bron_asset, right=df_relations_HoortBij, how='left', left_on='bron.@id', right_on='RelatieObject.bron.@id')
