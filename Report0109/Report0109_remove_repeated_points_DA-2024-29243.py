...
# import os
# import sys
# # Get the root folder path (parent of the current script's directory)
# root_folder = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
# # Add the root folder to the beginning of sys.path
...
from EMSONImporter import EMSONImporter  # Voordien EMInfraImporter, dit is gewijzigd naar EMSONImporter.
...
if __name__ == '__main__':
    ...
    # emson_importer = EMSONImporter(request_handler)
    eminfra_importer = EMInfraImporter(request_handler)



# Read the Excel file in a dataframe
excel_path = r"C:\Users\DriesVerdoodtNordend\Downloads\[RSA] Geometrie is geldig_ geen opeenvolgende punten_20241022.xlsx"
...
# object_generator = emson_importer.import_assets_from_webservice_by_uuids(asset_uuids=uuid)
...
# Filter the dataframe using a boolean mask. Filter on rows that have a geometry.
df_filtered = df[df['loc:Locatie.geometrie'] != '']

# Create a GeoSeries
gs_geometry = gpd.GeoSeries.from_wkt(df_filtered['loc:Locatie.geometrie'])
#print(f'Type: {type(gs_geometry)}')
#print(f'Geoseries: {gs_geometry}')

# Convert the DataFrame to a GeoDataFrame
gdf = gpd.GeoDataFrame(df_filtered, geometry=gs_geometry, crs="EPSG:31370")
...
os.makedirs('DA-2024-29243', exist_ok=True)
...
for type_name, items in type_groups.items():
    ...
    filepath = Path(f'DA-2024-29243/{type_name}.geojson')
